...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", type=str, default="../all_results_ablation_lshare_vtc.jsonl")
    ...
    # plot
    name = "rpm"
    if "vtc" in args.input:
        name = "rpm_vtc"
    plot(users, x_ticks, req_rate, "Time (s)", "Request Rate (token/s)", f"sec5.2_{name}_req_rate")
    plot(users, x_ticks, throughput, "Time (s)", "Throughput (token/s)", f"sec5.2_{name}_throughput")
    plot(users, x_ticks, response_time, "Time (s)", "Response Time (s)", f"sec5.2_{name}_response_time")
